  ...
  )
{
    ...
    while (m < numEpochs) {
      ...
      for (i = 0; i < numObjects; i++) {
        ...
        for (cd = 0; cd < numCodes; cd++) {
          ...
          if (tmp > 0) {
            threadCodeWeights[cd] += tmp * pObsWeights[i]; // pObsWeights ---> pObsWeights is a pointer, is it better to replace it with the original obsWeights ????
            for (j = 0; j < totalVars; j++) {
              if (!std::isnan(data[i * totalVars + j])) {
                threadCodeSums[cd * totalVars + j] +=
                  tmp * data[i * totalVars + j] * pObsWeights[i]; // pObsWeights ---> pObsWeights is a pointer, is it better to replace it with the original obsWeights ????
              }
            }
          }
        }
  ...
  RANDOUT;
    ...
    return RcppWeightedBatchSupersom(data, codes, numVars, weights, obsWeights,
           distanceFunctions, numNAs, neighbourhoodDistances, neighbourhoodFct,
			     radii, numEpochs);
  #endif
}
